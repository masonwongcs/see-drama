  ...
  if (res.status === 200) {
    ...
    res.data.forEach(async (value, index) => {
      ...
      $("body").append(dramaListWrapper).ready(function () {
        ...
        $(".placeholder").fadeOut().remove(); // carousel.on('dragStart.flickity', () => carousel.find('.slide').css('pointer-events', 'none'));
  ...
  if (location.pathname === "/" || location.pathname === "/drama") {
    ...
    $(document).on("click", ".drama-item", async function (e) {
      ...
      if (!isFetching) {
        ...
        let episodeList = $(`<div class="episode-list"></div>`);
        episodes.forEach(function (value, index) {
          let episodeItem = $(`<a href="/video?${value.url}" class="episode-item" data-video="/video?${value.url}"></a>`); // episodeItem.text(index + 1);
        ...
        if (location.pathname === "/") {
          ...
          $(this).closest(".drama-list").addClass("active").next(".episode-wrapper").append(episodeContainer).fadeIn();
        }
